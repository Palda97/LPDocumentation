...
\todo[inline]{A common issue that opponents point out is that it is not clear where the requirements came from. Like this, it looks like you came up with the requirements. However, typically, the requirements come from clients/future users, and they are gained in an interview. I would, therefore, say, that the requirements were gathered in an interview with the thesis supervisor, who is an experienced LP-ETL user.}
\section{Requirements}
In this section, atomic granular \todo[inline]{choose one of atomic and granular} requirements are gathered and described.

\subsection*{F-1.0: Settings screen}
Application must have a separate screen for settings. \todo{is it a good idea to number the requirements from 0? In text, I would number from 1}
...
\subsection*{F-3.1: Notification after finish \todo[inline]{pipeline}}
\label{subsec:notifications}
\todo[inline]{Here, it is not known to the user, what is a pipeline. Also, the thing that finishes, is a pipeline execution. Those terms need to be introduced in some sort of glossary before this section.}
...
\todo[inline]{item = pipeline execution}
